...
mksquashfs rootfs rootfs.squashfs -comp zstd -e boot -e initrd.img -e vmlinuz
...
tar -cf image_uefi.mnt/rootfs-overlay.tar --owner=root:0    --group=root:0    -C rootfs-overlay.tar.d/ etc/ root/
tar -rf image_uefi.mnt/rootfs-overlay.tar --owner=user:1000 --group=user:1000 -C rootfs-overlay.tar.d/ home/user/
gzip image_uefi.mnt/rootfs-overlay.tar
